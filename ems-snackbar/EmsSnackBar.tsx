...
export const EmsSnackbar = forwardRef<HTMLDivElement, EmsSnackbarProps>(({ id, message, variant, ...props }, ref) => {
  return (
    <SnackbarContent ref={ref} className="fixed top-[4.75rem] z-[999999] flex items-center justify-center">
      <div className="flex h-7 items-center gap-2 rounded-md border-[0.5px] border-neutral-4 bg-neutral-8 px-2 py-1">
        {variant == "success" && (
          <>
            ...
            <Image src={Close.src} width={20} height={20} alt="" className="cursor-pointer" onClick={() => onClickDismiss(id)} />
          </>
        )}
        {variant == "info" && (
          <>
